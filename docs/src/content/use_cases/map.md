...
<div class="container-xxl pt-3">
  <div class="card w-100">
    <div class="card-body">
      <h5 class="card-title pe-5">World map with SVG asset</h5>
      <p class="card-text pe-5">This use case illustrates how to load a world SVG map and use it as a geographic coordinate system for a bubble map.</p>
      <p class="card-text pe-5">
        The SVG map is loaded and registered before the chart is initialized:
        <code>
          <pre>
fetch('../../images/maps/world.svg')
  .then((response) =&gt; response.text())
  .then((svg) =&gt; echarts.registerMap('world-svg', { svg }));
          </pre>
        </code>
      </p>
      <button class="btn btn-icon btn-outline-secondary btn-edit" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Open in playground" data-additional-js-before="fetch('https://raw.githubusercontent.com/Orange-OpenSource/ods-charts/main/src/assets/world.svg').then((response) =&gt; response.text()).then((svg) =&gt; echarts.registerMap('world-svg', { svg })).then(() =&gt; {" data-additional-js-after="});">
      ...
      <div id="htmlId">
        <div class="border border-subtle position-relative">
          <div class="chart_title mx-3">
            <h4 class="display-4 mx-3 mb-1 mt-3">World connectivity</h4>
            <h5 class="display-5 mx-3 mb-1 mt-0">Traffic by city</h5>
          </div>
          <div id="map_holder">
            <div id="map_chart" style="width: 100%; height: 600px" class="position-relative"></div>
          </div>
        </div>
      </div>
      <script src="../../use_cases_code/map_codeId.js" id="codeId" onload="addViewCode()"></script>
    </div>
  </div>
</div>
<div class="container-xxl pt-3">
