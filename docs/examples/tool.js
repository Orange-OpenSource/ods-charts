...
export function generatedata(
  from = new Date('1971-01-26T10:15:30.996+0200'),
  to = new Date(),
  nbpoints = 1000,
  index = 0
) {
  ...
  while (date.getTime() < to.getTime()) {
    const value =
      date.getTime() /
        new Date(
          '' + (1971 + index * 5) + '-01-26T10:15:30.996+0200'
        ).getTime() +
      index +
...
export function getTimeSeriesData(
  nbPoint = 10000,
  nbSeries = 3,
  start = new Date('1971-01-26T10:15:30.996+0200'),
  end = new Date()
) {
  return Array.from({ length: nbSeries }, (val, index) =>
    generatedata(start, end, nbPoint, index)
  );
}
