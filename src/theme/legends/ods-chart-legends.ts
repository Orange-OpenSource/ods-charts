...
export class ODSChartsLegends {
    ...
    if (!dataOptions.legend || !dataOptions.legend.data) {
      if (dataOptions.dataset && dataOptions.dataset.source) {
        try {
          ...
          legendData = dataOptions.dataset.source[0].reduce(
  ...
  public addLegend(
    ...
    if (
      ...
      !document.querySelector('#ods-chart-legends-none-css')
    ) {
    ...
    if (!document.querySelector(this.legendHolderSelector)) {
      throw new Error(
        `No legend holder found with selector ${this.legendHolderSelector}`
      );
    }

    const legends: ODSChartsLegendData =
      ODSChartsLegends.getLegendData(dataOptions);
    ...
    if (!legendHolder) {
      throw new Error(
        `Can't find legend holder using the selector ${this.legendHolderSelector}`
      );
    }

    (document.querySelector(this.legendHolderSelector) as Element).innerHTML =
      this.generateLegend(colors, legends, cssTheme, mode);
  }
  ...
  private generateLegend(
    ...
    return `<div class="ods-charts-legend-holder ods-charts-mode-${mode} ${ODSChartsItemCSSDefinition.getClasses(
      cssTheme.legends?.odsChartsLegendHolder
    )}"
    style="${ODSChartsItemCSSDefinition.getStyles(
      cssTheme.legends?.odsChartsLegendHolder
    )}"
    >
    <div class="ods-charts-legend-container ${ODSChartsItemCSSDefinition.getClasses(
      cssTheme.legends?.odsChartsLegendContainer
    )}"
    style="${ODSChartsItemCSSDefinition.getStyles(
      cssTheme.legends?.odsChartsLegendContainer
    )}"
    ...
    ${(legends ? legends.labels : []).map(
      (
        legendLabel: string,
        index: number
      ) => `<a class="ods-charts-legend-link ${ODSChartsItemCSSDefinition.getClasses(
    ...
    <label class="ods-charts-legend-label ${ODSChartsItemCSSDefinition.getClasses(
      cssTheme.legends?.odsChartsLegendLabel
    )}"
    style="${ODSChartsItemCSSDefinition.getStyles(
      cssTheme.legends?.odsChartsLegendLabel
    )}"
  ...
  private generateHandler(cssTheme: ODSChartsCSSThemeDefinition) {
    ...
    (window as any).ods_chart_legend_switchLegend[this.legendHolderSelector] = (
      elt: HTMLElement,
      legendName: string
    ) => {
      ...
      const themeClasses =
        cssTheme.legends &&
        cssTheme.legends.odsChartsLegendLinkOpacity &&
